...
- `news`: provider-supplied headlines, source, URL, publication time, entities, language/country, snippets, and sentiment where returned, including EODHD, MarketAux, FMP, and Alpha Vantage `NEWS_SENTIMENT` feed items when status is ok.
...
| Tiingo | raw-only daily metadata and end-of-day prices returned by the configured starter account | `prices_daily`, `market_snapshot`, `technical_signals` |
...
- Estimate provider cost before network calls. If the estimated cost is above `--max-provider-calls PROVIDER=N` or the default conservative budget, skip the provider and write a manifest warning. Reusable cached endpoints (served without a live call) are never charged against the budget in either the cost estimate or the budget trim, so a tight budget keeps the cached endpoint in the plan and spends the remaining budget on the highest-priority uncached endpoints instead of being consumed by a call that never happens. A budget of zero still prevents live fetches, but reusable cached endpoints stay in the effective plan for normalization because they cost no calls. The budget-skipped and budget-zero per-provider metrics both record the estimated cost the provider would have incurred; the aggregate `provider_call_estimate` counts only providers that attempted a fetch.
- A `--provider-endpoints PROVIDER=...` filter may only name a provider that is in the run's `--providers` list. Naming a provider outside the run is a hard error (exit 2) rather than a silently added phantom that could win price-fetch selection and then fetch nothing; add it to `--providers` or drop the filter.
- Use endpoint-level planning to avoid duplicated daily price history. Daily prices stay plan-eligible for every configured price provider so normalization can fall back to any cached price series, but only one provider live-fetches prices per run: the highest-priority configured provider (Tiingo, then EODHD, then Alpha Vantage, then Twelve Data) that lacks a reusable cached price response **and** has enough remaining call budget to actually afford a `prices` call. A provider whose own budget cannot cover `prices` is skipped in favor of the next configured price provider, rather than being selected and then fetching nothing; if no configured provider can afford it and no cache covers it either, `fetch` records a "no live price fetch this run" manifest warning and `plan-fetch` exposes the same condition in top-level `warnings` with `price_fetch_provider: null`. When the top-priority provider already has usable cached prices, no provider live-fetches prices. Live daily-price calls for the remaining price providers are suppressed and recorded as a manifest warning plus `price_fetch_suppressed` in the fetch metrics and `plan-fetch` output; a provider whose own reusable cache covers `prices` is not marked suppressed. A suppressed provider's `prices` endpoint is excluded from its own cost estimate and budget trim, so its budget goes to its other endpoints instead of a call that will never be made. The default deterministic plan therefore fetches raw Tiingo metadata and Tiingo prices when Tiingo is configured, EODHD fundamentals/news plus raw historical market cap evidence without live EOD prices, Alpha Vantage overview/news sentiment plus raw statement/event/ETF profile evidence without live adjusted daily prices, Twelve Data quote/profile without live time series, MarketAux news, and FMP unique equity/ETF endpoints. Lower-priority price series are only fetched when a higher-priority provider is unavailable or explicitly excluded via `--providers`/`--provider-endpoints`. An explicit `--provider-endpoints PROVIDER=...prices...` filter opts that provider into live price fetching alongside the default selection, even if a higher-priority provider is also configured and fetching prices.
