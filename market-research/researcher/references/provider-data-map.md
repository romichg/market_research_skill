...
- Use endpoint-level planning to avoid duplicated daily price history. Daily prices stay plan-eligible for every configured price provider so normalization can fall back to any cached price series, but only one provider live-fetches prices per run: the highest-priority configured provider (Tiingo, then EODHD, then Alpha Vantage, then Twelve Data) that lacks a reusable cached price response. When the top-priority provider already has usable cached prices, no provider live-fetches prices. Live daily-price calls for the remaining price providers are suppressed and recorded as a manifest warning plus `price_fetch_suppressed` in the fetch metrics. The default deterministic plan therefore fetches raw Tiingo metadata and Tiingo prices when Tiingo is configured, EODHD fundamentals/news plus raw historical market cap evidence without live EOD prices, Alpha Vantage overview/news sentiment plus raw statement/event/ETF profile evidence without live adjusted daily prices, Twelve Data quote/profile without live time series, MarketAux news, and FMP unique equity/ETF endpoints. Lower-priority price series are only fetched when a higher-priority provider is unavailable or explicitly excluded via `--providers`/`--provider-endpoints`.
