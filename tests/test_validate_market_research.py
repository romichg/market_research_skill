...
def test_validator_data_bundle_sibling_report_lookup_preserves_dir_casing(tmp_path):
    run_dir = tmp_path / "data" / "aapl" / "2026-06-01"
    reports_dir = tmp_path / "reports" / "aapl" / "2026-06-01"
    normalized = run_dir / "normalized"
    normalized.mkdir(parents=True)
    ...
    validation = json.loads((tmp_path / "reports" / "AAPL" / "2026-06-01" / "AAPL-validation-scaffold.json").read_text(encoding="utf-8"))
    assert validation["report_json"] == str(reports_dir / "AAPL-research.json")
    assert validation["deterministic_data_usage_dispositions"]["missing_required"] == []
