...
def test_pdf_helper_explains_lmodern_when_pandoc_fails(tmp_path):
    ...
    status = json.loads(result.stdout.splitlines()[-1])
    assert status["generated"] is False
    assert status["reason"] == "pandoc_failed_missing_lmodern"


def test_pdf_helper_explains_missing_font_metric_when_pandoc_fails(tmp_path):
    report = tmp_path / "reports" / "ECH" / "2026-07-03" / "ECH-research.md"
    report.parent.mkdir(parents=True)
    report.write_text("# ECH Research\n", encoding="utf-8")
    bin_dir = tmp_path / "bin"
    bin_dir.mkdir()
    pandoc = bin_dir / "pandoc"
    pandoc.write_text(
        "#!/usr/bin/env bash\n"
        "echo 'mktextfm: `mf-nowin ... input pzdr'\"'\"' failed to make pzdr.tfm.' >&2\n"
        "echo 'Error producing PDF.' >&2\n"
        "echo '! Font \\\\XeTeXLink@font=pzdr at 0.00002pt not loadable: Metric (TFM) file or installed font not found.' >&2\n"
        "exit 1\n",
        encoding="utf-8",
    )
    xelatex = bin_dir / "xelatex"
    ...
    assert result.returncode == 0
    assert "PDF not generated" in result.stderr
    assert "font metric (TFM) file" in result.stderr
    assert "pzdr" in result.stderr
    assert "not lmodern" in result.stderr
    assert not report.with_suffix(".pdf").exists()
    status = json.loads(result.stdout.splitlines()[-1])
    assert status["generated"] is False
    assert status["reason"] == "pandoc_failed_missing_font_metric"
