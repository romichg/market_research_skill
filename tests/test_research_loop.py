...
def test_prompt_generation_mentions_fresh_contexts_and_artifact_contract(tmp_path):
    ...
    result = run_harness("write-prompts", "EWW", "--run-dir", "market-research-runs/EWW", "--output-dir", str(out_dir))
    ...
    assert "$market-research EWW" in producer
    assert "fresh Codex context" in validator
    assert "$validate-market-research market-research-runs/EWW" in validator
